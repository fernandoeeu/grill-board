          ...
          <SidebarGroup>
            <SidebarGroupContent>
              {activeTopics.length === 0 ? (
                <p className="px-2 py-1.5 text-sm leading-relaxed text-stone-400">No topics yet.</p>
              ) : (
                <SidebarMenu className="gap-0.5">
                  {activeTopics.map((topic) => (
                    <TopicItem key={topic.id} topic={topic} onToggleArchive={toggleArchived} />
                  ))}
                </SidebarMenu>
              )}
            </SidebarGroupContent>
...
function TopicItem({
  ...
  const isActive = Boolean(matchRoute({ to: "/topics/$topicId", params: { topicId: topic.id } }));
...
function TopicHoverDetails({ topic }: { topic: TopicSummary }) {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      {topic.context && (
        <p className="line-clamp-4 text-xs leading-relaxed text-stone-500">{topic.context}</p>
      )}
