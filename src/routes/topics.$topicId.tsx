...
function TopicBoard() {
  ...
  const sections = useMemo(() => groupByRound(questions ?? [], rounds ?? []), [questions, rounds]);
    ...
    return (
      <main className="mx-auto w-full max-w-3xl px-6 pt-24 pb-40 text-center">
        ...
        <p className="mt-4 text-sm text-stone-400">
          No topic with id <span className="font-mono text-stone-500">{topicId}</span>. Pick one
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-3xl px-6 pt-10 pb-40">
        {sections.length === 0 ? (
          <p className="mt-16 text-center text-sm text-stone-400">No questions yet.</p>
        ) : (
